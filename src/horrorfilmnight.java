...
public class horrorfilmnight{
    public static void main(String[] args) throws IOException{
        ...
        //Setup a and b in bitsets where a bit flipped on is a movie person a/b likes, respectively
        int an = in.nextInt();
        BitSet a = new BitSet();
        for(int i = 0;i < an;i++) a.set(in.nextInt());

        int bn = in.nextInt();
        BitSet b = new BitSet();
        for(int i = 0;i < bn;i++) b.set(in.nextInt());

        //Keep track of the state of the last watched movie, and the answer
        int last = -1, ans = 0;
        //Look through all the movies
        for(int i = 0;i < 1000000;i++){
            //Only process movies at least one of them like
            if(a.get(i) || b.get(i)){
                //Calculate state of this movie based on whether one person or both like it
                int next = a.get(i) && !b.get(i) ? 1 : (b.get(i) && !a.get(i) ? 2 : 3);
                //While the state is not the same as the last one watched, or if both like it, watch the movie
                if(last != next || next == 3){
                    last = next;
                    ans++;
                }
