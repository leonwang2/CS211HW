...
public class subseqhard{
    public static void main(String[] args) throws IOException{
        ...
        for(int test = 0;test < t;test++){
            ...
            //Keep track of amount of ways to make a certain value (key: value to be made, value: # ways to make)
            HashMap<Long, Integer> counts = new HashMap<>(n * 2);

            //There is 1 way to make 0
            counts.put(0L, 1);

            //Keep track of the current sum of all previous numbers and also total ways to make 47
            long sum = 0, total = 0;
            for(int i = 0;i < n;i++){
                //Read in next value in the sequence
                int val = in.nextInt();

                //Update sum
                sum += val;

                //Add counts of previous sums equal to the current sum - 47
                //Since that means there are that many ways of making a sequence adding up to 47
                if(counts.containsKey(sum - 47)) total += counts.get(sum - 47);

                //Add 1 to amount of ways to make the current sum
                if(counts.containsKey(sum)) counts.put(sum, counts.get(sum) + 1);
