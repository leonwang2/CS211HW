...
public class outofsorts{
    ...
    public static void main(String[] args) throws IOException{
        ...
        long n = in.nextInt(), m = in.nextInt(), a = in.nextInt(), c = in.nextInt(), first = in.nextInt();
        int[] arr = new int[(int)n];
        arr[0] = (int)((a * first + c) % m);
        for(int i = 1;i < n;i++) arr[i] = (int)((a * arr[i - 1] + c) % m);

        int found = 0;
        for(int i = 0;i < n;i++) if(find(arr, arr[i])) found++;
        out.println(found);
        in.close();
        out.close();
    }
