...
public class lektira{
    ...
    public static void split(String original, char[] tmp, int a, int b){
        int n = original.length();
        if(a < 1) for(int i = 1;i < n - 1;i++) split(original, tmp, i, b);
        else if(b < 1) for(int i = a + 1;i < n;i++) split(original, tmp, a, i);
        else{
            for(int i = 0;i < a;i++) tmp[i] = original.charAt(a - i - 1);
            for(int i = 0;i < b - a;i++) tmp[a + i] = original.charAt(b - i - 1);
            for(int i = 0;i < n - b;i++) tmp[b + i] = original.charAt(n - i - 1);

            for(int i = 0;i < n;i++){
                if(tmp[i] < min[i]){
                    System.arraycopy(tmp, 0, min, 0, n);
                    break;
                }else if(tmp[i] > min[i]) break;
            }
        }
    }
    ...
    public static void main(String[] args) throws IOException{
        ...
        min = new char[word.length()];
        Arrays.fill(min, 'z');
        split(word, new char[word.length()], 0, 0);
