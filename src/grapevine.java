...
public class grapevine{
    public static void main(String[] args) throws IOException{
        ...
        int[] skepticism = new int[n];
        for(int i = 0;i < n;i++){
            index.put(in.next(), i);
            skepticism[i] = in.nextInt();
        }

        ArrayList<Integer>[] edges = new ArrayList[n];
        for(int i = 0;i < n;i++) edges[i] = new ArrayList<>();
        for(int i = 0;i < m;i++){
            int a = index.get(in.next()), b = index.get(in.next());
            edges[a].add(b);
            edges[b].add(a);
        }
        ...
        int[] heard = new int[n];
        ArrayList<Integer> queue = new ArrayList<>(), buffer = new ArrayList<>();
        queue.add(index.get(in.next()));
        visited[queue.get(0)] = true;
        reached.add(queue.get(0));
        for(int day = 0;day < d;day++){
            for(int i = 0;i < queue.size();i++){
                int current = queue.get(i);
                for(int j = 0;j < edges[current].size();j++){
                    int next = edges[current].get(j);
                    if(visited[next]) continue;
                    heard[next]++;
                    reached.add(next);
                    if(heard[next] >= skepticism[next]){
                        buffer.add(next);
                        visited[next] = true;
                    }
                }
