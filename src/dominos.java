...
public class dominos{
    public static HashSet<Integer> origins = new HashSet<>();

    public static int dfs(ArrayList<Integer>[] edges, boolean[] visited, int[] origin, int current, int o){
        if(visited[current]) return origin[current];
        visited[current] = true;
        for(int i = 0;i < edges[current].size();i++){
            int from = dfs(edges, visited, origin, edges[current].get(i), o);
            if(from == o) origin[current] = o;
            if(from != o && from != -1) origins.remove(from);
        }
    ...
    public static void main(String[] args) throws IOException{
        ...
        for(int t = 0;t < tests;t++){
            ...
            for(int i = 0;i < m;i++) edges[in.nextInt() - 1].add(in.nextInt() - 1);
            ...
            for(int i = 0;i < n;i++){
                if(!visited[i]){
                    origins.add(i);
                    origin[i] = i;
                    dfs(edges, visited, origin, i, i);
                }
            }
            out.println(origins.size());
        }
