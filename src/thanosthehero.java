...
public class thanosthehero{
    public static void main(String[] args) throws IOException{
        ...
        //Start from the end to find the minimum amount of kills
        for(int i = n - 1;i > 0;i--){

            //If the smaller index world has more or equal population than a larger one, kill enough people to make it not so (as Thanos prefers the larger index world and so it must have more population)
            if(pops[i - 1] >= pops[i]){
                int remove = pops[i - 1] - pops[i] + 1;

                //Add the population Thanos kills to the counter and remove it from the array
                kills += remove;
                pops[i - 1] -= remove;
            }

            //Thanos cannot kill people to make the population negative, so Thanos kills himself
            if(pops[i - 1] < 0){
                kills = 1;
                break;
            }
