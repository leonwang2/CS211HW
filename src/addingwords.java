...
public class addingwords{
    public static void main(String[] args) throws IOException{
        ...
        HashMap<Integer, String> valToVar = new HashMap<>();
        HashMap<String, Integer> varToVal = new HashMap<>();
        ...
        while(in.hasNext()){
            ...
            if(command.charAt(1) == 'e'){
                ...
                if(varToVal.containsKey(var)) valToVar.remove(varToVal.get(var));
                valToVar.put(val, var);
                ...
                while(op != '='){
                    ...
                    if(!varToVal.containsKey(var)) known = false;
                    else if(known) value += varToVal.get(var) * (op == '-' ? -1 : 1);
                    op = in.nextChar();
                    result.append(op).append(' ');
                }

                if(!valToVar.containsKey(value) || !known) result.append(unknown);
                else result.append(valToVar.get(value));

                out.println(result.toString());
            }else if(command.charAt(1) == 'l'){ //clear has an 'l' there but not def and calc
                varToVal.clear();
                valToVar.clear();
            }
        }
