...
public class alphabet{
    public static void main(String[] args) throws IOException{
        ...
        String str = in.next();
        int n = str.length();
        //Set up dp array where dp[i] represents the longest alphabetical subset possible in the substring from i to the end
        int[] dp = new int[n];
        //Initialize base case; Each letter is it's own subset of length 1
        Arrays.fill(dp, 1);
        //Recurse back from the end
        for(int i = n - 2;i >= 0;i--){
            for(int j = i + 1;j < n;j++){
                //If the letter at j is after the letter at i, the letter at i can become a part of the subset starting at j
                if(str.charAt(j) > str.charAt(i)) dp[i] = Math.max(dp[i], dp[j] + 1);
            }
        }
        //Calculate answer, 26 minus the maximum alphabetical subset length in the dp array
        int ans = 0;
