...
public class driver{
    public static void main(String[] args) throws IOException{
        ...
        Arrays.sort(indexes, Comparator.comparingInt(i -> costs[i]));
        LinkedList<Integer> waits = new LinkedList<Integer>();
        for(int i = 0;i < n - 1;i++){
            int r = indexes[i];
            if(i == 0 || r < waits.peek()) waits.addFirst(r);
        }
        waits.addLast(n - 1);

        int[] times = new int[n];
        for(int i = 0;i < n;i++) times[i] = in.nextInt();

        long cost = 0;
        int current = waits.poll(), time = 0;
        while(!waits.isEmpty()){
            int nextStop = waits.poll(), maxWait = 0;
            long totalCost = 0;

            for(int i = current;i < nextStop;i++){
                totalCost += costs[i];
                maxWait = Math.max(maxWait, times[i + 1] - (i - current) - 1);
            }
            while(time < maxWait){
                time += 2;
                cost += costs[current] * 2L;
            }

            cost += totalCost;
            time += nextStop - current;
            current = nextStop;
        }
        out.println(cost);
        in.close();
