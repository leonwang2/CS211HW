...
public class runningmom{
    ...
    public static int city(String name){
    ...
    public static void dfs(int current){
        ...
        for(int i = 0;paths[current] != null && i < paths[current].size();i++){
            int next = paths[current].get(i);
            if(!visited[next]) dfs(next);
            deadEnd = deadEnd && end[next];
        }
        end[current] = deadEnd;
    }
    ...
    public static void main(String[] args) throws IOException{
        ...
        for(int i = 0;i < n;i++){
            int a = city(in.next()), b = city(in.next());
            if(paths[a] == null) paths[a] = new ArrayList<>();
            paths[a].add(b);
        }

        visited = new boolean[cities.size()];
        end = new boolean[cities.size()];

        for(int i = 0;i < cities.size();i++) if(!visited[i]) dfs(i);

        while(in.hasNext()){
            int next = city(in.next());
            out.print(cities.get(next));
            out.print(" ");
            out.println(end[next] ? "trapped" : "safe");
        }
