...
public class imagedecoding{
    public static void main(String[] args) throws IOException{
        ...
        //The maximum amount of characters of each line will never exceed 1000
        char[] line = new char[1000];
        int n = in.nextInt();

        //Iterate until end (when n = 0)
        while(n != 0){
            boolean error = false;
            ...
            for(int i = 0;i < n;i++){
                //Reset index to zero (it's the next line)
                index = 0;

                //Find out whether to start with a period or not
                boolean period = in.nextChar() == '.';

                //Iterate until the end of the line of input
                while(!in.lineEnd()){
                    int next = in.nextInt();

                    //Set the next chars in the line of the image to periods/hashtags
                    for(int j = 0;j < next;j++){
                        line[index++] = period ? '.' : '#';
                    }

                    //Change to the other symbol (periods/hashtags)
                    period = !period;
                }

                //Print out the line of the image
                out.println(new String(line, 0, index));

                //Index now effectively holds the length of the line we just printed, so if its not the same as the last index, there is an error
                //"i" must be > 0 because at 0 there is no last index for comparison
                if(lastIndex != index && i > 0){
                    error = true;
                }

                //Store the last index for comparison
                lastIndex = index;
            }
            ...
            //Read the next input and if there is one then seperate the printed images with a newline
            n = in.nextInt();
            if(n != 0) out.println();
        }
        in.close();
