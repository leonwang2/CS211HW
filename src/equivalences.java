...
public class equivalences{
    ...
    public static void main(String[] args) throws IOException{
        ...
        for(int t = 0;t < tests;t++){
            ...
            BitSet hasFrom = new BitSet(n), hasTo = new BitSet(n);
            for(int i = 0;i < n;i++){
                ArrayList<Integer> c = paths[i];
                for(int j = 0;j < c.size();j++){
                    int a = group[i], b = group[c.get(j)];
                    if(a == b) continue;
                    hasFrom.set(a);
                    hasTo.set(b);
                }
            ...
            for(int i = 0;i < n;i++){
                ...
                if(!hasFrom.get(i)) fromCount++;
                if(!hasTo.get(i)) toCount++;
                groups++;
            }
            out.println(groups == 1 ? 0 : Math.max(fromCount, toCount));
        }
        in.close();
        out.close();
    }
