...
public class logland{
    public static void main(String[] args) throws IOException{
        ...
        //Keep track of answer and the current power of 2 (for answer calculations)
        long ans = 0, pow = 1;
        //Keep track of how many coins of the i-1 denomination it is possible to make
        long makes = 0;
        for(int i = 0;i < n;i++){
            //Keep track of how many coins of the ith denomination it is possible to make
            long next = 0;
            //Update next
            if(i > 0) next = (makes + vals[i - 1]) / 2;
            //If number of ith coin denominations is odd, and there is no way to make one a coin of the ith denomination, we must leave this coin
            if(vals[i] % 2 == 1 && next == 0) ans = (ans + pow) % prime;
            //Update the power of 2
            pow = (pow * 2) % prime;
            //Keep track of next (as it becomes the i-1 denomination)
            makes = next;
        }
        out.println(ans);
        in.close();
